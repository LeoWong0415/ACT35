...
BYTE ChangeRGBPosH()
{
	...
	value = GetRGBPosH();
	SaveHActiveEE(InputVAdcMode,value);
	return value;
}
#endif
...
BYTE ChangeRGBPosV(void)
{
	...
	value = GetRGBPosV();
	SaveVBackPorchEE(InputVAdcMode,value);
	return value; 
}
#endif
...
//		WORD GetVAdcPhase(void)
//		WORD SetVAdcPhase(WORD val)
//		BYTE ChangeVAdcPhase(void)
//=============================================================================
//interface between MENU and VAdc
#if defined(SUPPORT_PC)
//-----------------------------------------------------------------------------
/**
...
*/
BYTE VaryVAdcPhase(BYTE val)
{
	BYTE old,value;

	old = GetVAdcPhase();	//0..100
	value = VAdcGetPhase();	//0..0x1F
	...
	VAdcSetPhase(value,ON);	//WithInit
...
BYTE SetVAdcPhase(BYTE val)
{
	...
	VAdcSetPhase((BYTE)value, ON); //WithInit
	return GetVAdcPhase();
}
...
BYTE ChangeVAdcPhase(void)
{
	...
	val = VAdcGetPhase(); //0..0x1F

	SavePhaseEE(InputVAdcMode, val);

	val = GetVAdcPhase(); //0..100
	return val;
}
#endif
...
//		WORD GetVAdcClock(void)
//		WORD SetVAdcClock(WORD val)
//		BYTE ChangeVAdcClock(void)
//=============================================================================
//interface between MENU and VAdc
...
* @return 0..100
*/
BYTE GetVAdcClock(void) //GetVAdcClock
{
	BYTE bTemp;
	WORD wTemp;
	if(InputVAdcMode==0)
	...
	//return (BYTE)(50+PcBaseClock-VAdcLLPLLGetDivider());

	wTemp =	VAdcLLPLLGetDivider();
	PcBaseClock = PCMDATA[ InputVAdcMode ].htotal-1;
	bTemp = 50+VAdcLLPLLGetDivider()-PcBaseClock;
...
BYTE SetVAdcClock(BYTE val)
{
	...
	VAdcLLPLLSetDivider(clock, 0);	//without INIT
...
BYTE ChangeVAdcClock(void)
{
	...
	u_val = GetVAdcClock(); //0..100
	SavePixelClkEE(InputVAdcMode, u_val);
	return u_val;	
}
#endif
...
void SetImage(BYTE type)
{
	...
	if(type == INPUT_PC ||type == INPUT_DVI || type == INPUT_HDMIPC) {
...
*/
BYTE OsdGetTransRate(void) //0..100
{
	return OsdTransRate;
}
//-----------------------------------------------------------------------------
...
*/
BYTE OsdSetTransRate(BYTE dat) //0..100
{
	if(dat <= 100)
		OsdTransRate = dat;
