...
//	dMenuPrintf("\nMenuReadRleHeader:%06lx:",spi_loc);
	...
	if(header->id[0]!='I' || header->id[1]!='T') {
		...
		dPrintf(" MenuReadRleHeader fail");
		return 1;	//fail
	}
	//swap
	wTemp = header->w;
...
//	dMenuPrintf(" dummy %bx %bx",header->dummy0, header->dummy1);
	dPrintf("\nloc:%06lx", spi_loc);
	dPrintf(" op:%02bx",header->op_bmpbits); 
	dPrintf(" rle:%02bx",header->rledata_cnt);
	dPrintf(" wh:%dx%d",header->w, header->h);
	dPrintf(" size:%lx", header->size);
	dPrintf(" LUT format:%bx color:%bx", header->LUT_format,header->LUT_colors);
	dPrintf(" dummy %bx %bx",header->dummy0, header->dummy1);
...
//Printf("\n	lut_size:%x",header->lut_size);
	...
	SpiOsdWinScreen( WIN_LOGO, 0/*sx*/, 0/*sy*/, header->dx, header->dy );
	if(WIN_LOGO==0) {
	...
	if(header->rle) {	//need RLE ?
		//SpiOsdEnableRLC(ON);
		SpiOsdRLC( WIN_LOGO, header->bpp,header->rle);
	}	
	else {
		//BK110203
	...
	//Load Palette
	SpiOsdLoadLUT(WIN_LOGO, header->lut_type, lut_loc, header->lut_size, header->lut_loc);

	//pixel alpha blending. after load Palette
	if(image->alpha != 0xFF) {
		SpiOsdPixelAlphaAttr(lut_loc+image->alpha, 0x7F);
	}
	// finish draw
}
...
//	lut_size = 0x004 << (image->rle >> 4);
	...
	//pixel alpha ???
	if(image->alpha != 0xFF) {
		SpiOsdPixelAlphaAttr(0 /*menu_item->osd_s.lut*/ +image->alpha, 0x7F);
	}
}
...
void MenuDrawCurrImage(BYTE use1, BYTE item)
{
	...
	if(UseSOsdHwBuff) {
		...
		//pixel alpha blending. after load Palette
		if(image->alpha != 0xFF)
			SOsdHwBuffSetAlpha(menu_item->lut+image->alpha);
	}
	else {
		//WaitVBlank(1);
		if(header->rle) {	//need RLE ?
		...
		//Load Palette
		SpiOsdLoadLUT(sosd_win, header->lut_type, menu_item->lut, header->lut_size, header->lut_loc);
		...
		//pixel alpha blending. after load Palette
		if(image->alpha != 0xFF) {
			SpiOsdPixelAlphaAttr(menu_item->lut+image->alpha, 0x7F);
		}
	}
}
...
void MenuDrawScrollImage(BYTE item, BYTE direction)
{
	...
	WaitVBlank(1);
	SpiOsdLoadLUT(sosd_win, header->lut_type, menu_item->lut, header->lut_size, header->lut_loc);	
	if(image->alpha != 0xFF) {	 //pixel alpha ???
		SpiOsdPixelAlphaAttr(menu_item->lut+image->alpha, 0x7F);
	}
	...
	if(direction==NAVI_KEY_DOWN) {		
		for(i=1; i <= header->dy; i++) {
			...
			SpiOsdWinScreen(sosd_win, 0,0, header->dx, i);
	...
	else if(direction==NAVI_KEY_RIGHT)
	{
		...
		for(i=1; i <= header->dx; i++) {
			...
			SpiOsdWinImageLoc(sosd_win, header->image_loc + start_byte);
			SpiOsdWinImageLocBit(sosd_win, start_bit);
			SpiOsdWinScreen(sosd_win, 0,0, i, header->dy);
			//delay
			WaitVBlank(1);
			SOsdWinBuffWrite2Hw(menu_item->win, menu_item->win);
...
void proc_slider_ok(void)
{
	...
	if(slide_title_img==&img_slide_clock) {
		//VAdcLLPLLSetDivider(,1);
		WriteTW88Page(0x00);
		WriteTW88(REG003, 0xF8);	//enable ISR.
		VAdcSetFilterBandwidth(7, 0);
	}
...
#ifdef SUPPORT_BT656
		case INPUT_BT656:	ChangeBT656();		break;
#endif
		}		
		//??SpiOsdSetDeValue();
		//??FOsdSetDeValue();
...
void proc_osd_trans(void) 
{
	...
	OsdSetTransRate(EE_Read(EEP_OSD_TRANSPARENCY));
	InitSlideCtrl(&img_slide_trans, &OsdGetTransRate,&OsdSetTransRate,&OsdSetTransRate,&OsdChangeTransRate);	  //BKTODO: init slide control
...
void proc_wait_menu_start(void)
{
	...
	WaitVBlank(1);
	SpiOsdLoadLUT(SOSD_WIN0, header->lut_type, menu_item->lut, header->lut_size, header->lut_loc);
...
void MenuStart(void)
{
	...
	//Read EE values
	//example:
	//	MenuInputMain = GetInputMain();	//do not use, InputMain = GetInputEE(); in menu.c
	...
	osd_lang = 0;	//read OSDLANG  //BKTODO:USE EE
...
void DParkGridAction(BYTE n)
{
	...
	WaitVBlank(1);
//PORT_DEBUG=0;
	SpiOsdRLC( WIN_GRID_IMG, rle_byte >> 4, rle_byte & 0x0F);
	SpiOsdLoadLUT(WIN_GRID_IMG, 1, 0, TEST_GRID_LUT_LEN, lGridStartAddr); //Load Palette : type, LUT offset, size, address
