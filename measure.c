...
WORD MeasGetHTotal(BYTE hPol)
{
	...
	if(hPol) {
		...
		if(MeasGetInputClkForHPeriodReg()==0) {
			dTemp = SspllGetPPF(0);	//SspllGetFreqReg();
...
void AutoTunePhase(void)
{
	...
	if ( RGBmin > 10 ) {
		...
		VAdcSetPhase(OldPhase,0);	//restore Old Phase value.
	}
	else {
		//select big.
		VAdcSetPhase(j, 0);
...
void AutoTuneColor(void)
{
	...
	GainG = VAdcReadGChannelGainReg();
	GainB = VAdcReadBChannelGainReg();
	GainR = VAdcReadRChannelGainReg();
	if(nGainG != GainG
	|| nGainB != GainB
	|| nGainR != GainR) 
		VAdcSetChannelGainReg(nGainG,nGainB,nGainR);
