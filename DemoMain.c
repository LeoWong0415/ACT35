...
void PigeonDemo( void )
{
	...
	WaitVBlank(1);
	SpiOsdLoadLUT( 0, 0/*1*/, 0,0x400, PIGEON_LUT_LOC );	//win0
...
void RoseDemo( void )
{
	...
	WaitVBlank(1);
	SpiOsdLoadLUT(0, 0/*1*/, 0, 0x400, ROSE_LUT_LOC );	//win0
	SOsdWinBuffWrite2Hw(0, 0);
	dPuts("\nFinished LUTRoseDMA");
...
#define WIN_GRID_IMG	1	//with RLE
...
void MovingGridInit( void )
{
	...
	//need RLE
	SpiOsdRLC( WIN_GRID_IMG, d_grid_rle[0]>>4,d_grid_rle[0]&0x0F); //(curr_bg_info->rle >> 4), curr_bg_info->rle & 0x0F);
	...
	WaitVBlank(1);
	SpiOsdLoadLUT(0, LUTTYPE_ADDR, 0, 0x400,  DGRID_LUT_START ); //win ?0
	WaitVBlank(1);
	SOsdWinBuffWrite2Hw(WIN_GRID_MSG, WIN_GRID_IMG);
	//test only IMG
...
void MovingGridLUT( BYTE n )
{
	...
	offset *= 1024;
	WaitVBlank(1);
	SpiOsdLoadLUT( 0 ,LUTTYPE_ADDR, 0, 0x400, DGRID_LUT_START + offset );	//win ?0
